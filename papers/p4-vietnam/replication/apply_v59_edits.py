...
Applies 20 paragraph-level rewrites across:
- Abstract (4 blocks): Purpose, Design, Findings, Originality
- §2.1 H1: dual-margin participation+intensity framing
- §2.4 H4: tightened to wave-specific exploratory framing
- §4.3 hypothesis tests (2 paragraphs)
- §5.1 Reinterpreting digital capability (2 paragraphs)
- §5.2 TCI vs DAI (3 paragraphs)
- §5.3 2015 dip (3 paragraphs)
- §6 Limitations (2 paragraphs: limit2 + limit4)
- §7 Conclusion (2 paragraphs)

Inputs:
    submission/manuscript_blinded_FIXED.docx (= v5.8 + 7 prior fixes from fix_p4_v58.py)
...
For with-authors version, run with sed substitution:
    sed 's|manuscript_blinded_FIXED|manuscript_full_with_authors_FIXED|g; \\
         s|manuscript_v5_9_blinded|manuscript_v5_9_full_with_authors|g' \\
        apply_v59_edits.py > apply_v59_full.py
    python3 apply_v59_full.py
"""
import shutil
from docx import Document

SRC = "submission/manuscript_blinded_FIXED.docx"
DST = "submission/manuscript_v5_9_blinded.docx"
...
# (prefix to match, new full paragraph text, label)
EDITS = [
    # --- Abstract (4 blocks) ---
...
for prefix, new_text, label in EDITS:
    if replace_para_by_prefix(doc, prefix, new_text):
        ...
        applied.append(f'  ✗ {label}: NOT FOUND')

doc.save(DST)
print('=== v5.9 edits applied ===')
for a in applied:
    print(a)
print(f'\nTotal: {sum(1 for a in applied if "✓" in a)}/{len(EDITS)} edits applied')
print(f'Output: {DST}')
import os
